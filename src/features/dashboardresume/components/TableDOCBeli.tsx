...
const columns = [
  ...
  { id: "supplier", label: "Supplier", sort: true },
];
const datas = [
  {
    kode_barang: "FWFW",
...
const TableDOCBeli = ({ checkYear }: { checkYear: any }) => {
  // const { data, isLoading } = useSalesDoc({ params: { checkYear } });
  ...
  return (
    <section className="bg-white mb-8 rounded-xl px-4 md:px-6 lg:px-10 py-6 text-xs font-medium gap-y-8 border border-black border-opacity-20">
      <div className="text-black opacity-60 font-medium text-xl">DOC Beli</div>
      <Table2
        header={columns}
        items={datas}
