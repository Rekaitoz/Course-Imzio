...
const TableHPPResume = ({ docData, feedmillData, ovkData }: any) => {
  ...
  //   (docData[index][1] * (1 + dataChange[index].row[index1].dpls) +
  //   dataChange[index].row[index1].fcr *
  //     dataChange[index].row[index1].abw *
  //     feedmillData[index][1] +
  //   ovkData[index][1]) /
  // dataChange[index].row[index1].abw
  ...
  useEffect(() => {
    if (dataChange) {
      setDataChange(
        docData.map((item: { tahun: string; row: RowData[] }, index: any) => {
          return {
            tahun: item.tahun,
            row: new Array(3).fill(1).map((rowData, index1) => ({
              1:
                (docData[index][1] * (1 + dataChange[index].row[index1].dpls) +
                  dataChange[index].row[index1].fcr *
                    dataChange[index].row[index1].abw *
                ...
                dataChange[index].row[index1].abw,
              2:
                (docData[index][2] * (1 + dataChange[index].row[index1].dpls) +
                  dataChange[index].row[index1].fcr *
                    dataChange[index].row[index1].abw *
                    feedmillData[index][2] +
                  ovkData[index][2]) /
                dataChange[index].row[index1].abw,
              3:
                (docData[index][3] * (1 + dataChange[index].row[index1].dpls) +
                  dataChange[index].row[index1].fcr *
                    dataChange[index].row[index1].abw *
                    feedmillData[index][3] +
                  ovkData[index][3]) /
                dataChange[index].row[index1].abw,
              4:
                (docData[index][4] * (1 + dataChange[index].row[index1].dpls) +
                  dataChange[index].row[index1].fcr *
                    dataChange[index].row[index1].abw *
                    feedmillData[index][4] +
                  ovkData[index][4]) /
                dataChange[index].row[index1].abw,
              5:
                (docData[index][5] * (1 + dataChange[index].row[index1].dpls) +
                  dataChange[index].row[index1].fcr *
                    dataChange[index].row[index1].abw *
                    feedmillData[index][5] +
                  ovkData[index][5]) /
                dataChange[index].row[index1].abw,
              6:
                (docData[index][6] * (1 + dataChange[index].row[index1].dpls) +
                  dataChange[index].row[index1].fcr *
                    dataChange[index].row[index1].abw *
                    feedmillData[index][6] +
                  ovkData[index][6]) /
                dataChange[index].row[index1].abw,
              7:
                (docData[index][7] * (1 + dataChange[index].row[index1].dpls) +
                  dataChange[index].row[index1].fcr *
                    dataChange[index].row[index1].abw *
                    feedmillData[index][7] +
                  ovkData[index][7]) /
                dataChange[index].row[index1].abw,
              8:
                (docData[index][8] * (1 + dataChange[index].row[index1].dpls) +
                  dataChange[index].row[index1].fcr *
                    dataChange[index].row[index1].abw *
                    feedmillData[index][8] +
                  ovkData[index][8]) /
                dataChange[index].row[index1].abw,
              9:
                (docData[index][9] * (1 + dataChange[index].row[index1].dpls) +
                  dataChange[index].row[index1].fcr *
                    dataChange[index].row[index1].abw *
                    feedmillData[index][9] +
                  ovkData[index][9]) /
                dataChange[index].row[index1].abw,
              10:
                (docData[index][10] * (1 + dataChange[index].row[index1].dpls) +
                  dataChange[index].row[index1].fcr *
                    dataChange[index].row[index1].abw *
                    feedmillData[index][10] +
                  ovkData[index][10]) /
                dataChange[index].row[index1].abw,
              11:
                (docData[index][11] * (1 + dataChange[index].row[index1].dpls) +
                  dataChange[index].row[index1].fcr *
                    dataChange[index].row[index1].abw *
                    feedmillData[index][11] +
                  ovkData[index][11]) /
                dataChange[index].row[index1].abw,
              12:
                (docData[index][12] * (1 + dataChange[index].row[index1].dpls) +
                  dataChange[index].row[index1].fcr *
                    dataChange[index].row[index1].abw *
                    feedmillData[index][12] +
                  ovkData[index][12]) /
                dataChange[index].row[index1].abw,
              abw: dataChange[index].row[index1].abw,
              fcr: dataChange[index].row[index1].fcr,
              dpls: dataChange[index].row[index1].dpls,
            })),
          };
        })
      );
